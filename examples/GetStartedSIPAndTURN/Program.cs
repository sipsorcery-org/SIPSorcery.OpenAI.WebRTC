...
class Program
{
    private static int DEFAULT_SIP_LISTEN_PORT = 5060;

    private static string? _turnUrl = string.Empty;
    private static int _sipListenPort = DEFAULT_SIP_LISTEN_PORT;
    ...
    static async Task Main()
    {
        ...
        var sipPortStr = Environment.GetEnvironmentVariable("SIP_LISTEN_PORT");
        if (int.TryParse(sipPortStr, out var sipPort))
        {
            _sipListenPort = sipPort;
            Log.Logger.Information($"SIP listen port set to {_sipListenPort}.");
        }

        SIPSorcery.LogFactory.Set(loggerFactory);
        var sipTransport = new SIPTransport();
        sipTransport.EnableTraceLogs();
        sipTransport.AddSIPChannel(new SIPUDPChannel(new IPEndPoint(IPAddress.Any, _sipListenPort)));
...
public static class TurnServerExtensions
{
    public static RTCIceServer ParseTurnServer(this string turnServer)
    {
        var fields = turnServer.Split(';');
